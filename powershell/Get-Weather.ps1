...
$API = "0a1eaf717f684a908be06d7219eb22b4"
...
$resultsJSON = Invoke-WebRequest "api.openweathermap.org/data/2.5/weather?q=$City,$Country&units=imperial&appid=$API&type=accurate&mode=json"
...
[xml]$results = Invoke-WebRequest "api.openweathermap.org/data/2.5/weather?q=$City,$Country&units=imperial&appid=$API&type=accurate&mode=xml"
...
$currentTemp = "Current Temp: " + [Math]::Round($data.temperature.value, 0) + [char] 176 + "f"
$high = "Today's High: " + [Math]::Round($data.temperature.max, 0) + [char] 176 + "f"
$low = "Today's Low: " + [Math]::Round($data.temperature.min, 0) + [char] 176 + "f"
